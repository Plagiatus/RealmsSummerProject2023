...
execute if entity @s[tag=tr_husk_close,tag=!tr_hiding] run damage @e[tag=tr_close_husk,scores={tr_husk_cd=1..},limit=1] 0 player_attack by @s
execute if entity @s[tag=tr_husk_close,tag=!tr_hiding] run scoreboard players set @e[tag=tr_close_husk,scores={tr_husk_cd=1..}] tr_husk_cd 0
# Player is currently hidden - husk will loose interest in attacking them, goes on sniffing cooldown
execute if entity @s[tag=tr_husk_close,tag=tr_hiding] run scoreboard players add @e[tag=tr_close_husk] tr_husk_cd 1
